...
为什么需要这个脚本
------------------
原先由 `07_orchestration/modal_full_pipeline.py` 的 `run_phase1_only` 承担，
但那是「本地入口 → 一个编排容器 → 容器内 `.map()` 420 条边」的结构，存在两个致命弱点：

1. `--detach` 只保活**最后触发的**那个函数。编排容器一旦被抢占，
   其内部 map 的全部输入会被批量取消（2026-08-27 03:54 实测：
   "Worker disappeared" → 数百条 "Received a cancellation signal"）。
2. 无断点。即便给编排容器加 retries，重试也是从零开始，
   已算完的边全部作废——单边 25–55 分钟，代价极高。

本脚本改为：**每条边算完立刻把结果写入 Volume**，
再由合并步骤读取全部分片做 FDR。任何中断只损失在途的边，重跑自动跳过已完成的。
