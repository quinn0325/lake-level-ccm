"""重算全部湖泊、全部变量的嵌入参数，写出 results/embed_params_corrected.json。
...
输出
----
    results/embed_params_corrected.json
...
def main():
    ...
    local_out = CODE_DIR.parent.parent / "results" / "embed_params_corrected.json"
    local_out.parent.mkdir(parents=True, exist_ok=True)
    local_out.write_text(payload, encoding="utf-8")
    print(f"已写入本地:   {local_out}")
