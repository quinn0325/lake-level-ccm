...
`figures/` holds every figure in the dissertation as PNG. Rerunning a figure
script writes PDF, SVG and TIFF alongside it in `results/figures/`; only the PNGs
are committed. File names follow the script rather than the numbering in the text:
...
`appendices/` holds the eleven appendix tables as CSV. They are printed in the
dissertation itself, so only the data is kept here -- it is what `verify.py`
regenerates and compares against. `code/07_tables/build_appendices_en.py`
re-renders them as Markdown if you want the formatted version.
