...
与 modal_connectivity_full_pairwise.py 的区别（后者保留备查，不再作为主路径）
--------------------------------------------------------------------------
1. **统一显著性判定**：原脚本在 main() 里内联计算 FDR 与 causal_evidence，
   因此拿不到 `apply_fdr_and_causal_evidence` 中新增的**时序保留规则**
   （d>0 保留 / d=0 标注 unresolved / d<0 不作因果证据）。
   本脚本改为调用共享库的同一函数，湖内湖间口径完全一致。
2. **断点续跑**：每条边算完立刻写入 Volume 分片，中断只损失在途的边。
3. **spawn_map 提交**：不用 `.map()`——后者阻塞等待且默认
   return_exceptions=False，任一输入永久失败即抛异常、本地入口崩溃，
   Modal 随即取消其余全部在途任务（2026-08-27 实测多次因此全批作废）。
4. **不跑 PCMCI**：按 2026-08-26 决定，PCMCI 不纳入正文，
   湖间混杂控制改由「有水道连接 vs 无水道连接」的分组对照承担。
   原脚本的 PCMCI 部分保留在该文件中备查。
