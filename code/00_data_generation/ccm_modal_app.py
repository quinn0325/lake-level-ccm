...
跑法分两步（main()里自动做，不用手动分开调用）：
  ...
  第2步 process_lake_modal：ERA5已经缓存到Volume，15个容器全速并行跑CCM(pairwise-complete)。

要过夜/合盖跑（本地机器全程不用在线），用run_detached——两步的调度逻辑本身也在云端执行：
  modal run --detach ccm_modal_app.py::run_detached
派发完几秒内本地就退出，之后合盖/断网都不影响云端把两步跑完。也可以用fetch_only/process_only
分开手动跑（见各自函数docstring），或者main()一条命令跑完两步但要求本地进程全程存活
（配合caffeinate，适合"电脑开着过一夜"但不合盖的场景）。
...
def audit_masks_modal() -> list:
    """核实第1步"掩膜/站点选择有没有问题"：对15个湖泊逐一算出Hylak_id解析结果、湖泊掩膜
    (T/Evap用)和流域掩膜(P/R/SWE用)各自命中多少个ERA5-Land格点、参照站点到HydroLAKES记录的
    真实出水口(Pour_long/Pour_lat)之间的距离——不跑CDS/WSC/pyEDM，很快。

    这几个数字process_lake_modal内部算过但没存进checkpoint(跟combined_wl一样的疏漏)，这里
    单独补算一次，纯粹用于核实，不影响正式流程的checkpoint。
    """
    import glob
    from pathlib import Path
    ...
    from ccm_lib import resolve_lake_area_bbox
    ...
    for lake_name in LAKES:
        ...
        # 一个湖泊的resolve_lake_area_bbox抛异常不该让整个审计循环连其余湖泊都查不到——
        # 踩过一次(Prosperous_Lake参照点没落进任何子流域多边形直接把整个函数炸掉)。
        try:
            hylak_id, lake_cells, basin_cells, area_bbox = resolve_lake_area_bbox(lake_name, hydrolakes_gdf, hydrobasins_gdf)
        except Exception as e:
            rows.append({"lake": lake_name, "status": f"FAILED_exception: {type(e).__name__}: {e}"})
            print(f"{lake_name}: [EXCEPTION] {type(e).__name__}: {e}")
            continue
        if hylak_id is None:
            rows.append({"lake": lake_name, "status": "FAILED_hylak_id_not_resolved"})
            continue
        pour_row = hydrolakes_gdf.loc[hydrolakes_gdf["Hylak_id"] == hylak_id].iloc[0]
        pour_pt = Point(pour_row["Pour_long"], pour_row["Pour_lat"])
        ref_pt = Point(ref_lonlat)
        # 粗略的度数距离，不是精确的地表距离，只用来判断"参照站点离HydroLAKES记录的出水口
        # 是不是明显偏远"这个量级问题，不追求测地精度。
        dist_deg = ref_pt.distance(pour_pt)

        rows.append({
            "lake": lake_name,
            "hylak_id": hylak_id,
            "hydrolakes_name": pour_row.get("Lake_name"),
            "hydrolakes_area_km2": round(float(pour_row["Lake_area"]), 2),
            "reference_station": LAKE_OUTLET_STATION.get(lake_name) or LAKES[lake_name]["stations"][0],
            "ref_station_all_stations": LAKES[lake_name]["stations"],
            "n_stations_used_for_wl": len(LAKES[lake_name]["stations"]),
            "n_lake_cells": len(lake_cells),
            "n_basin_cells": len(basin_cells),
            "ref_to_pour_point_deg": round(dist_deg, 4),
        })
        print(f"{lake_name}: hylak_id={hylak_id}, HydroLAKES名={pour_row.get('Lake_name')}, "
              f"面积={pour_row['Lake_area']:.1f}km², 湖泊格点={len(lake_cells)}, 流域格点={len(basin_cells)}, "
              f"参照站点到出水口距离={dist_deg:.4f}度")
    return rows


@app.function(image=image, volumes={DATA_ROOT: volume}, timeout=600)
def search_hydrolakes_by_name_modal(search_term: str, near_lonlat: str = "") -> list:
    """按名字关键词(不区分大小写、子串匹配)在HydroLAKES里搜索候选多边形，列出Hylak_id、
    面积、国家、出水口坐标——不下结论选哪个，人工核对。near_lonlat可选，格式"lon,lat"，
    传了的话额外算一下每个候选离这个参照点的距离，帮助判断哪个候选更靠谱。"""
    import glob
    from pathlib import Path
    ...
    mask = hydrolakes_gdf["Lake_name"].astype(str).str.contains(search_term, case=False, na=False)
    candidates = hydrolakes_gdf[mask].copy()

    ref_pt = None
    if near_lonlat:
        from shapely.geometry import Point
        lon, lat = map(float, near_lonlat.split(","))
        ref_pt = Point(lon, lat)
        candidates["dist_to_ref_deg"] = candidates.geometry.distance(ref_pt)
        candidates = candidates.sort_values("dist_to_ref_deg")

    rows = []
    for _, row in candidates.iterrows():
        centroid = row.geometry.centroid
        r = {
            "Hylak_id": int(row["Hylak_id"]), "Lake_name": row["Lake_name"], "Country": row.get("Country"),
            "Lake_area_km2": round(float(row["Lake_area"]), 2),
            "Pour_long": row["Pour_long"], "Pour_lat": row["Pour_lat"],
            "centroid_lon": round(centroid.x, 4), "centroid_lat": round(centroid.y, 4),
        }
        if ref_pt is not None:
            r["dist_to_ref_deg"] = round(float(row["dist_to_ref_deg"]), 4)
        rows.append(r)
        print(f"  Hylak_id={r['Hylak_id']}: {r['Lake_name']} ({r['Country']}), "
              f"面积={r['Lake_area_km2']}km², 质心=({r['centroid_lon']}, {r['centroid_lat']})"
              + (f", 离参照点{r['dist_to_ref_deg']}度" if ref_pt is not None else ""))
    return rows
...
@app.function(image=image, volumes={DATA_ROOT: volume}, timeout=600)
def audit_regflow_modal() -> list:
    """核实第①步剩下的部分：RegFlow调控站点选择有没有问题。对12个有REGULATION_STATIONS条目
    的湖泊，逐一算出每个调控站到HydroLAKES记录出水口(Pour_long/Pour_lat)的距离——跟audit_masks_modal
    对WL参照站点做的是同一套量级核查，只是对象换成调控站。不跑CDS/WSC/pyEDM，很快。

    调控站本身语义跟WL站不同：WL站要贴着湖泊本体，调控站要贴着水坝/放水口(通常在湖泊出水口
    下游一小段河道上)，所以距离量级预期会比WL站略大一点，不能直接套用同一个阈值下结论，
    这里只是把量化数字摆出来，具体每个站是否合理仍需要结合REGULATION_STATIONS里的中文注释
    (哪些已经核实过、哪些是替代站)人工判断。
    """
    import glob
    from pathlib import Path
    ...
    rows = []
    for lake_name, station_ids in REGULATION_STATIONS.items():
        hylak_id, lake_cells, basin_cells, area_bbox = resolve_lake_area_bbox(lake_name, hydrolakes_gdf, hydrobasins_gdf)
        if hylak_id is None:
            rows.append({"lake": lake_name, "status": "FAILED_hylak_id_not_resolved"})
            continue
        ...
        for sid in station_ids:
            ref_pt = Point(STATION_COORDS[sid])
            dist_deg = ref_pt.distance(pour_pt)
            rows.append({
                "lake": lake_name,
                "regflow_station": sid,
                "hylak_id": hylak_id,
                "hydrolakes_name": pour_row.get("Lake_name"),
                "n_regflow_stations_for_lake": len(station_ids),
                "regflow_to_pour_point_deg": round(dist_deg, 4),
                "subperiod_restricted": f"{sub[0]}-{sub[1]}" if sub else "",
            })
            print(f"{lake_name} [{sid}]: 离HydroLAKES出水口({pour_row.get('Lake_name')})距离="
                  f"{dist_deg:.4f}度" + (f", 子区间限制={sub[0]}-{sub[1]}" if sub else ""))
    return rows
...
def process_lake_modal(lake_name: str, n_surrogates: int = 200, sample: int = 100) -> dict:
    ...
    from ccm_lib import fetch_lake_water_level, fetch_lake_regulation_flow, resolve_lake_area_bbox, \
        ...
        convert_era5_units, build_variable_panel, get_embedding_params, build_full_ccm_table, \
        run_lag_scans, build_causal_network, classify_edges, find_common_drivers, \
        find_transitive_mediation, select_sarimax_candidates
    ...
    try:
        ...
        panel_raw, panel_deseason = build_variable_panel(combined_wl, {c: real_predictors[c] for c in real_predictors.columns}, forecast_horizon=FORECAST_HORIZON)
        result["panel_raw"] = panel_raw.dropna()
        ...
        ccm_table = build_full_ccm_table(ccm_train_panel, embed_params, sample=sample, n_surrogates=n_surrogates, seed=0, verbose=False)
        lag_results = run_lag_scans(ccm_train_panel, embed_params, ccm_table, lags=range(-12, 13), seed=0)
        G = build_causal_network(ccm_table)
        edge_classification = classify_edges(G, lag_results=lag_results)
        common_drivers_df = find_common_drivers(G)
        transitive_df = find_transitive_mediation(G)
        sarimax_candidates = select_sarimax_candidates(ccm_table, lag_results, common_drivers_df)

        result.update({
            "status": "OK", "embed_params": embed_params, "ccm_table": ccm_table, "lag_results": lag_results,
            "edge_classification": edge_classification, "common_drivers_df": common_drivers_df,
            "transitive_df": transitive_df, "sarimax_candidates": sarimax_candidates,
        })
    except Exception as e:
        import traceback
...
@app.function(
    image=image,
    volumes={DATA_ROOT: volume},
    secrets=[modal.Secret.from_name("cds-api")],  # 兜底用，正常情况下ERA5已缓存不会真的打CDS
    timeout=600,
    max_containers=15,
)
def audit_data_quality_modal(lake_name: str) -> dict:
    """第②步：真正的逐变量缺失值/物理范围/月度跳变/日历缺口体检——不读process_lake_modal存的
    checkpoint(那份`panel_raw`在ccm_modal_app.py:543被`.dropna()`过，只反映"全部变量listwise
    重叠窗口"，看不出RegFlow等变量自己实际缺了哪几个月)，而是重新跑一遍跟process_lake_modal
    完全一样的数据抓取+面板构建逻辑(ERA5已缓存，不会重新打CDS；WSC是轻量HTTP GET)，
    在dropna之前先做体检，这样才能看到每个变量各自真实的缺测模式。
    跟正式CCM流程(process_lake_modal)完全独立，不touch checkpoint/PIPELINE_VERSION。
    """
    import glob
    from pathlib import Path
    ...
    from ccm_lib import (
        fetch_lake_water_level, fetch_lake_regulation_flow, resolve_lake_area_bbox,
        try_fetch_era5_land, extract_masked_series, convert_era5_units,
        build_variable_panel, deseasonalize,
    )
    ...
    result = {"lake_name": lake_name, "status": "started"}
    try:
        combined_wl, wide_wl, coverage_wl = fetch_lake_water_level(lake_name, method="anomaly_mean")
        regflow_series = fetch_lake_regulation_flow(lake_name)
        hylak_id, lake_cells, basin_cells, area_bbox = resolve_lake_area_bbox(lake_name, hydrolakes_gdf, hydrobasins_gdf)
        ...
        panel_raw, panel_deseason = build_variable_panel(combined_wl, {c: real_predictors[c] for c in real_predictors.columns}, forecast_horizon=FORECAST_HORIZON)
        # 故意不dropna——这里就是要看每个变量自己真实缺了哪些月份。

        missing_by_var = {var: int(panel_raw[var].isna().sum()) for var in panel_raw.columns}
        n_total_months = len(panel_raw)

        gap_rows_by_var = {}
        for var in panel_raw.columns:
            s = panel_raw[var].dropna()
            if len(s) == 0:
                gap_rows_by_var[var] = None
                continue
            full_range = pd.date_range(s.index.min(), s.index.max(), freq="MS")
            missing = full_range.difference(s.index)
            gap_rows_by_var[var] = {
                "n_present": len(s), "span_months": len(full_range), "n_gap_months": len(missing),
                "first": s.index.min().strftime("%Y-%m"), "last": s.index.max().strftime("%Y-%m"),
                "gap_dates": list(missing.strftime("%Y-%m")) if len(missing) else [],
            }
        ...
        result.update({
            ...
            "gap_rows_by_var": gap_rows_by_var,
        })
    except Exception as e:
        result["status"] = f"FAILED_exception: {type(e).__name__}: {e}"
    return result
...
def connectivity_ccm_modal() -> dict:
    """第14节"湖间连通性分析"：检验CONNECTIVITY_CHAINS里每条链上相邻两个湖泊之间，
    上游湖泊水位是否因果驱动下游湖泊水位(WL->WL)。

    跟notebook原版一处不同：原版对整条链(比如Kalamalka->Okanagan->Vaseux三个湖)一次性做
    listwise dropna再取相邻两两组合；这里改成每一对相邻湖泊单独按pairwise-complete原则
    (Little & Rubin, 2002)取重叠区间——跟这次给主CCM流程做的修复是同一个设计哲学，不因为
    链条里某个湖泊数据短就拖累其他两两组合。

    直接用fetch_lake_water_level重新抓水位(走WSC网站，不是CDS，不受ERA5那个限流问题影响，
    也很便宜)，不依赖process_lake_modal的checkpoint里有没有存combined_wl(它目前没存)。
    """
    from pathlib import Path
    ...
    from ccm_lib import (
        fetch_lake_water_level, get_embedding_params, ccm_edge_curve, _pair_panel,
        surrogate_significance, convergence_test, lag_scan,
    )

    def deseasonalize(series, train_end=None):
        """与 ccm_lib.deseasonalize 一致：气候态只用训练窗口计算，避免测试期泄漏。
        调用方须传入 train_end=len(series)-FORECAST_HORIZON。"""
    ...
    involved_lakes = sorted({lake for chain in CONNECTIVITY_CHAINS.values() for lake in chain})
    lake_wl = {}
    for lake_name in involved_lakes:
        combined_wl, wide_wl, coverage_wl = fetch_lake_water_level(lake_name, method="anomaly_mean")
        lake_wl[lake_name] = (lambda _s: deseasonalize(_s, train_end=len(_s) - FORECAST_HORIZON))(combined_wl.sort_index().asfreq("MS"))

    rows = []
    for chain_name, chain_lakes in CONNECTIVITY_CHAINS.items():
        for a, b in zip(chain_lakes[:-1], chain_lakes[1:]):
            panel = pd.concat([lake_wl[a].rename(a), lake_wl[b].rename(b)], axis=1)
            panel = panel.iloc[:-FORECAST_HORIZON]  # 跟主流程口径一致，留出最后12个月给最终测试
            embed_params = {lake: get_embedding_params(panel[lake].values, lake, verbose=False) for lake in (a, b)}

            for cause, effect in [(a, b), (b, a)]:
                sig = surrogate_significance(panel, cause, effect, embed_params, n_surrogates=200, seed=0)
                pair_n = sig["n_span"]  # 完整日历跨度，不是n_obs(实际两列都有观测的月份数)
                lib_sizes = f"{max(10, pair_n // 3)} {pair_n} 10"
                pair_df = _pair_panel(panel, [cause, effect])
                curve = ccm_edge_curve(pair_df, cause, effect, embed_params, lib_sizes, sample=100, seed=0)
                conv = convergence_test(curve)
                scan = lag_scan(panel, cause, effect, embed_params, lags=range(-12, 13), seed=0)
                l_star = int(scan.loc[scan["rho"].idxmax(), "lag"]) if len(scan) else None
                rows.append({"chain": chain_name, **sig, **conv, "l_star_months": l_star})
                print(f"  {cause} -> {effect}: rho={sig['obs_rho']:.3f}, p={sig['p_value']:.4f}, "
                      f"n_obs={sig['n_obs']}, convergence={conv['convergence_diagnostic_pass']}, l*={l_star}")

    table = pd.DataFrame(rows)
    _, fdr_p, _, _ = multipletests(table["p_value"], alpha=0.05, method="fdr_bh")
    table["p_fdr"] = fdr_p
    table["significant"] = table["p_fdr"] < 0.05

    out_path = Path(DATA_ROOT) / "lake_results" / "connectivity_ccm_results.csv"
    table.to_csv(out_path, index=False)
    volume.commit()

    print("\n=== 湖间连通性CCM结果（FDR校正后）===")
    print(table[["chain", "cause", "effect", "obs_rho", "p_fdr", "significant", "n_obs", "l_star_months"]].to_string(index=False))

    return {"n_pairs_tested": len(rows), "n_significant": int(table["significant"].sum())}
...
@app.function(
    ...
    volumes={DATA_ROOT: volume},
    timeout=1800,
    max_containers=15,  # 30对湖泊，15个容器并行跑，每个容器算一对(两个方向)
)
def connectivity_pair_modal(group: str, lake_a: str, lake_b: str) -> dict:
    """检验同一生态区内一对湖泊之间的WL->WL因果关系(两个方向都测)。每对湖泊单独按
    pairwise-complete原则(Little & Rubin, 2002)取重叠区间，不受其他湖泊对影响。
    FDR校正需要收集全部结果之后才能做，留给connectivity_ccm_ecoregion这个入口做。"""
    import pandas as pd
    ...
    wl = {}
    for lake_name in (lake_a, lake_b):
        combined_wl, wide_wl, coverage_wl = fetch_lake_water_level(lake_name, method="anomaly_mean")
        wl[lake_name] = (lambda _s: deseasonalize(_s, train_end=len(_s) - FORECAST_HORIZON))(combined_wl.sort_index().asfreq("MS"))

    panel = pd.concat([wl[lake_a].rename(lake_a), wl[lake_b].rename(lake_b)], axis=1)
    panel = panel.iloc[:-FORECAST_HORIZON]
    embed_params = {lake: get_embedding_params(panel[lake].values, lake, verbose=False) for lake in (lake_a, lake_b)}

    rows = []
    for cause, effect in [(lake_a, lake_b), (lake_b, lake_a)]:
        sig = surrogate_significance(panel, cause, effect, embed_params, n_surrogates=200, seed=0)
        # lib_sizes按n_span(两列共同覆盖的完整日历跨度)算，不是n_obs(实际两列都有观测的
        # 月份数)——ccm_lib.py重写后这两个含义分开了，n_span才是CCM library size该用的分母。
        pair_n = sig["n_span"]
        lib_sizes = f"{max(10, pair_n // 3)} {pair_n} 10"
        pair_df = _pair_panel(panel, [cause, effect])
        curve = ccm_edge_curve(pair_df, cause, effect, embed_params, lib_sizes, sample=100, seed=0)
        ...
        scan = lag_scan(panel, cause, effect, embed_params, lags=range(-12, 13), seed=0)
        l_star = int(scan.loc[scan["rho"].idxmax(), "lag"]) if len(scan) else None
        row = {"group": group, "cause": cause, "effect": effect, **sig, **conv, "l_star_months": l_star}
        rows.append(row)
        print(f"  [{group}] {cause} -> {effect}: rho={sig['obs_rho']:.3f}, p={sig['p_value']:.4f}, "
              f"n_obs={sig['n_obs']}, l*={l_star}")
    return {"rows": rows}
...
def write_ecoregion_results_to_volume(content: bytes):
    from pathlib import Path
    out_path = Path(DATA_ROOT) / "lake_results" / "connectivity_ccm_ecoregion_results.csv"
...
@app.function(image=image, volumes={DATA_ROOT: volume})
def write_final10_connectivity_results_to_volume(content: bytes):
    from pathlib import Path
    out_path = Path(DATA_ROOT) / "lake_results" / "connectivity_ccm_final10_results.csv"
    out_path.write_bytes(content)
    volume.commit()
...
@app.function(image=image, volumes={DATA_ROOT: volume})
def write_chains_connectivity_results_to_volume(content: bytes):
    from pathlib import Path
    out_path = Path(DATA_ROOT) / "lake_results" / "connectivity_ccm_chains_results.csv"
    out_path.write_bytes(content)
    volume.commit()


@app.function(image=image, volumes={DATA_ROOT: volume}, timeout=1800, max_containers=15)
def connectivity_pair_residual_modal(group: str, lake_a: str, lake_b: str) -> dict:
    """排除局地气候共同驱动混淆的稳健性检验：跟connectivity_pair_modal逻辑完全一致，
    唯一区别是WL用的是wl_climate_residual.py算好、存在Volume上lake_results/wl_residuals.csv
    里的"局地气候(T/P/R/SWE/Evap)解释不了的那部分WL残差"，不是原始去季节化WL。如果某一对
    湖泊在原始WL上显著、但在残差上不显著了，说明那条连通性大概率是共同气候驱动的假象，不是
    真实的水力/调控关联；如果残差上依然显著，说明背后还有气候解释不了的真实关联。"""
    import pandas as pd
    from pathlib import Path

    from ccm_lib import (
        get_embedding_params, ccm_edge_curve, _pair_panel,
        surrogate_significance, convergence_test, lag_scan,
    )

    resid_path = Path(DATA_ROOT) / "lake_results" / "wl_residuals.csv"
    resid_df = pd.read_csv(resid_path, index_col=0, parse_dates=True)

    panel = resid_df[[lake_a, lake_b]].copy()
    panel = panel.iloc[:-FORECAST_HORIZON]  # 跟主流程/connectivity_pair_modal同一套留出窗口口径
    embed_params = {lake: get_embedding_params(panel[lake].values, lake, verbose=False) for lake in (lake_a, lake_b)}
    ...
    for cause, effect in [(lake_a, lake_b), (lake_b, lake_a)]:
        ...
        row = {"group": group, "cause": cause, "effect": effect, **sig, **conv, "l_star_months": l_star}
        rows.append(row)
        print(f"  [{group}][residual] {cause} -> {effect}: rho={sig['obs_rho']:.3f}, p={sig['p_value']:.4f}, "
              f"n_obs={sig['n_obs']}, l*={l_star}")
    return {"rows": rows}
...
@app.function(image=image, volumes={DATA_ROOT: volume})
def write_final10_residual_connectivity_results_to_volume(content: bytes):
    from pathlib import Path
    out_path = Path(DATA_ROOT) / "lake_results" / "connectivity_ccm_final10_residual_results.csv"
    out_path.write_bytes(content)
    volume.commit()
...
def forecast_compare_modal(lake_name: str) -> dict:
    """在同一个最终测试窗口(留出的最后FORECAST_HORIZON个月，不参与任何调参)上，把几种
    预测方法摆在一起直接对比：
      - SARIMA：纯WL自身历史，不用任何外生变量
      - Climatology：逐月气候态
      - SARIMAX_CCM_top：CCM找到的FDR显著候选里rho最高的那一个变量+它的l_star滞后
      - SARIMAX_CCM_ancestors：WL在CCM显著因果网络里的**全部祖先**(networkx.ancestors，任意跳数，
        不限1跳/2个——取代了之前"top_plus_paths(中介路径最多2个)"的做法，是它的严格超集)。
        只要存在一条全部由FDR显著边组成的有向路径最终指向WL，这个变量就纳入；每个纳入变量的
        滞后跟SARIMAX_all_vars一样现场重新跑lag_scan(不查缓存的lag_results，因为纯多跳祖先
        可能根本没有直接边，缓存里没有它对WL的滞后扫描结果)，只要求l_star>=0。
      - SARIMAX_all_vars："不管CCM显著性"对照组——全部可用变量都放进去，每个变量的滞后
        仍然用CCM算出来的l_star(即使没过FDR校正)，只要求l_star>=0(方向合理，不用"未来"
        预测"现在")。SARIMAX_CCM_ancestors是它的子集(只挑跟WL有因果路径的那些变量)。
      - XGBoost_AR_only/XGBoost_CCM_top/XGBoost_CCM_ancestors/XGBoost_all_vars：跟上面4个
        SARIMA(X)方法逐一对应的树模型版本(fit_xgboost_multi)，变量集合/滞后完全复用同一份
        (top_candidate/ancestor_lags/all_var_lags)，只是换一种模型拟合——这样能把"变量选择
        方法"和"模型家族(线性SARIMAX vs 非线性树模型)"两个维度分开比较，2×4的完全对照设计。
        跟SARIMAX同一套hindcast实验设计(外生变量/WL自身滞后都用真实历史值，不做递归多步
        预测)，树模型超参数固定为保守默认值，不逐湖调参(避免n~350个月的小样本被调参过拟合)。

    数据来源：process_lake_modal已经缓存在Volume的{lake}_result.pkl，不重新跑CCM显著性
    检验(200次IAAFT替代数据太贵)，但WL基准序列改用wide_wl重新合成(不是用checkpoint里
    listwise dropna过的panel_raw)，避免RegFlow这类局部缺测变量把WL自己的可用窗口也拖累——
    跟这次给CCM主流程做的pairwise-complete修复是同一个道理，也是SARIMAX_all_vars所有
    变量各自real_predictors的滞后取值一样按各自的NaN模式处理，不受RegFlow等变量拖累。
    """
    import pickle
    from pathlib import Path
    ...
    from ccm_lib import (
        build_variable_panel, combine_station_water_levels, fetch_lake_water_level, deseasonalize,
        lag_scan, build_causal_network, _forecast_metrics,
        fit_auto_sarima, fit_auto_sarimax_multi, fit_climatology, fit_xgboost_multi,
    )

    RESULTS_DIR = Path(DATA_ROOT) / "lake_results"
    checkpoint_path = RESULTS_DIR / f"{lake_name}_result.pkl"
    if not checkpoint_path.exists():
        return {"lake_name": lake_name, "status": "FAILED_no_cached_ccm_result"}
    with open(checkpoint_path, "rb") as f:
        cached = pickle.load(f)
    if cached.get("status") != "OK":
        return {"lake_name": lake_name, "status": f"FAILED_ccm_status_{cached.get('status')}"}

    # WL重新从wide_wl合成(不是checkpoint里listwise dropna过的panel_raw["WL"])，
    # 这样WL自身的可用窗口不受RegFlow等其他变量局部缺测拖累。
    combined_wl = combine_station_water_levels(cached["wide_wl"], method="anomaly_mean")
    real_predictors = cached["real_predictors"]
    panel_raw, panel_deseason = build_variable_panel(combined_wl, {c: real_predictors[c] for c in real_predictors.columns}, forecast_horizon=FORECAST_HORIZON)
    ccm_train_panel = panel_deseason.iloc[:-FORECAST_HORIZON]  # 跟process_lake_modal同一套训练窗口口径

    embed_params = cached["embed_params"]
    available_vars = [v for v in embed_params if v != "WL"]

    # ---- 补算"全部变量"各自相对WL的l_star(不筛FDR，只要求l_star>=0) ----
    all_var_lags = {}
    for var in available_vars:
        scan = lag_scan(ccm_train_panel, var, "WL", embed_params, lags=range(-12, 13), seed=0)
        if scan.empty:
            continue
        l_star = int(scan.loc[scan["rho"].idxmax(), "lag"])
        if l_star >= 0:
            all_var_lags[var] = l_star

    # ---- CCM候选(复刻notebook _select_forecast_features_from_training的选择逻辑) ----
    sarimax_candidates = cached["sarimax_candidates"]
    valid = sarimax_candidates[sarimax_candidates["l_star_months"].notna() & (sarimax_candidates["l_star_months"] >= 0)]
    valid = valid.sort_values("rho_to_WL", ascending=False)
    top_candidate = None
    if not valid.empty:
        top_row = valid.iloc[0]
        top_candidate = {"variable": top_row["variable"], "lag": int(top_row["l_star_months"])}

    # ---- WL在CCM显著因果网络里的全部祖先(任意跳数)，滞后现场重新跑lag_scan(理由见上面docstring) ----
    G = build_causal_network(cached["ccm_table"])
    ancestor_vars = sorted(nx.ancestors(G, "WL")) if "WL" in G.nodes() else []
    ...
    for var in ancestor_vars:
        ...
        l_star = int(scan.loc[scan["rho"].idxmax(), "lag"])
        if l_star >= 0:
            ancestor_lags[var] = l_star

    wl_series = panel_deseason["WL"].sort_index().asfreq("MS")
    test_size = FORECAST_HORIZON

    empty_exog = panel_deseason.iloc[:, :0]  # 0列的DataFrame，配合exog_lags={}时表示纯AR基线

    # ---- 邻居湖泊水位(connectivity_ccm_chains里FDR显著的cause->本湖泊边)，检验"知道邻居
    # 湖泊水位能不能帮这个湖泊自己预测得更准"。邻居列名加"_neighbor_"前缀避免跟本湖泊
    # 自己的变量(比如两个湖都有RegFlow)重名；邻居WL同样做去季节化，跟本湖泊WL处理口径一致。
    neighbor_exog_cols = {}
    neighbor_lags = {}
    for neighbor_name, lag in NEIGHBOR_PREDICTORS.get(lake_name, []):
        col = f"_neighbor_{neighbor_name}"
        try:
            neighbor_combined_wl, _, _ = fetch_lake_water_level(neighbor_name, method="anomaly_mean")
            neighbor_wl = (lambda _s: deseasonalize(_s, train_end=len(_s) - FORECAST_HORIZON))(neighbor_combined_wl.sort_index().asfreq("MS"))
            neighbor_exog_cols[col] = neighbor_wl.reindex(panel_deseason.index)
            neighbor_lags[col] = lag
        except Exception as e:
            print(f"  [{lake_name}] 邻居湖泊{neighbor_name}水位抓取失败，跳过: {type(e).__name__}: {e}")
    neighbor_panel = pd.DataFrame(neighbor_exog_cols) if neighbor_exog_cols else None

    methods = {}
    methods["SARIMA"] = lambda: fit_auto_sarima(wl_series, test_size=test_size)
    methods["Climatology"] = lambda: fit_climatology(wl_series, test_size=test_size)
    methods["XGBoost_AR_only"] = lambda: fit_xgboost_multi(wl_series, empty_exog, {}, test_size=test_size)
    if top_candidate is not None:
        top_exog_lags = {top_candidate["variable"]: top_candidate["lag"]}
        methods["SARIMAX_CCM_top"] = lambda: fit_auto_sarimax_multi(
            wl_series, panel_deseason[[top_candidate["variable"]]], top_exog_lags, test_size=test_size)
        methods["XGBoost_CCM_top"] = lambda: fit_xgboost_multi(
            wl_series, panel_deseason[[top_candidate["variable"]]], top_exog_lags, test_size=test_size)
    if ancestor_lags:
        methods["SARIMAX_CCM_ancestors"] = lambda: fit_auto_sarimax_multi(
            wl_series, panel_deseason[list(ancestor_lags.keys())], ancestor_lags, test_size=test_size)
        methods["XGBoost_CCM_ancestors"] = lambda: fit_xgboost_multi(
            wl_series, panel_deseason[list(ancestor_lags.keys())], ancestor_lags, test_size=test_size)
    if all_var_lags:
        methods["SARIMAX_all_vars"] = lambda: fit_auto_sarimax_multi(
            wl_series, panel_deseason[list(all_var_lags.keys())], all_var_lags, test_size=test_size)
        methods["XGBoost_all_vars"] = lambda: fit_xgboost_multi(
            wl_series, panel_deseason[list(all_var_lags.keys())], all_var_lags, test_size=test_size)
    if neighbor_panel is not None and not neighbor_panel.empty:
        # CCM_ancestors(本湖泊自己的显著变量)再叠加邻居湖泊水位，检验"知道邻居湖泊水位
        # 能不能在本湖泊自己的变量之外再提升预测精度"——不是替代CCM_ancestors，是它的超集。
        neighbor_combined_exog = pd.concat(
            [panel_deseason[list(ancestor_lags.keys())], neighbor_panel], axis=1)
        neighbor_combined_lags = {**ancestor_lags, **neighbor_lags}
        methods["SARIMAX_neighbor_lake"] = lambda: fit_auto_sarimax_multi(
            wl_series, neighbor_combined_exog, neighbor_combined_lags, test_size=test_size)
        methods["XGBoost_neighbor_lake"] = lambda: fit_xgboost_multi(
            wl_series, neighbor_combined_exog, neighbor_combined_lags, test_size=test_size)

    # 单次固定切分(74个月)，在同一个测试期上按多个预见期(1/3/6/12个月)分别截断评估——
    # 复用已经生成的完整预测轨迹，不重新训练模型。这个做法直接对应水位预测文献里的实际
    # 常见做法(而不是逐起点滚动重新训练再平均那种time series CV)：Xu et al. (2023, Water
    # 15(3):576，鄱阳湖Transformer)训练期1960-2000/测试期2001-2013只切一次，但在同一个
    # 固定测试期上分别报告1/2/3/4/5/6/7/15/30天这些不同预见期的精度；Alipour Modab (2025,
    # Water Sci Technol 92(12):1650，水库水位)也是单次80/20切分。两篇都不是逐起点滚动
    # 重新拟合的time series cross-validation，是本研究实际采用的设计依据。
    HORIZONS = [1, 3, 6, 12]
    ...
    for name, fn in methods.items():
        try:
            r = fn()
            results[name] = {"rmse": r["rmse"], "mae": r["mae"], "nse": r["nse"],
                              "pbias": r.get("pbias"), "n_eval": r["n_eval"]}
            if "vif" in r and r["vif"] is not None:
                results[name]["vif_max"] = float(r["vif"].max()) if len(r["vif"].dropna()) else None

            by_horizon = {}
            actual_full, pred_full = r.get("actual"), r.get("predicted")
            if actual_full is not None and pred_full is not None:
                for h in HORIZONS:
                    if h > len(actual_full):
                        continue
                    m = _forecast_metrics(actual_full.iloc[:h].values, pred_full.iloc[:h].values)
                    by_horizon[f"h{h}"] = {"rmse": m["rmse"], "mae": m["mae"], "nse": m["nse"],
                                            "pbias": m.get("pbias"), "n_eval": m["n_eval"]}
            results[name]["by_horizon"] = by_horizon

            print(f"  [{lake_name}] {name}: RMSE(full)={r['rmse']:.4f}, NSE(full)={r['nse']:.3f}"
                  + "".join(f" | h{h}:RMSE={by_horizon[f'h{h}']['rmse']:.4f}" for h in HORIZONS if f"h{h}" in by_horizon))
        except Exception as e:
            failures.append({"method": name, "error": f"{type(e).__name__}: {e}"})
            print(f"  [{lake_name}] {name}: 失败 - {type(e).__name__}: {e}")

    return {
        "lake_name": lake_name, "status": "OK", "results": results, "failures": failures,
        "top_candidate": top_candidate, "ancestor_lags": ancestor_lags, "all_var_lags": all_var_lags,
...
@app.function(image=image, volumes={DATA_ROOT: volume})
def write_forecast_compare_to_volume(content: bytes):
    from pathlib import Path
    out_path = Path(DATA_ROOT) / "lake_results" / "forecast_compare_results.csv"
    out_path.write_bytes(content)
    volume.commit()
...
@app.function(
    ...
    volumes={DATA_ROOT: volume},
    secrets=[modal.Secret.from_name("cds-api")],
    timeout=21600,  # 6小时：ERA5预下载(~30分钟) + CCM计算(pairwise-complete之后单湖泊上限2小时,
                    # 15个并行但仍要给足余量)，两步加起来留足空间。
)
def run_full_pipeline_modal(lakes_csv: str = "") -> dict:
    """在Modal云端内部完整跑完两步(先ERA5预下载，再CCM计算)——注意这是普通@app.function，
    不是@app.local_entrypoint，函数体本身就跑在Modal容器里，"等第1步完成再触发第2步"这个
    调度逻辑也在云端执行，不依赖本地进程存活。配合run_detached()用.spawn()异步派发，
    本地派发完就能立刻退出/合盖，不影响这个函数在云端继续跑完两步。"""
    target_lakes = lakes_csv.split(",") if lakes_csv else list(LAKES.keys())

    print(f"[云端调度] 第1步：低并发预下载ERA5，{len(target_lakes)}个湖泊: {target_lakes}")
    era5_results = list(fetch_era5_modal.map(target_lakes))
    for r in era5_results:
        print(f"  {r['lake_name']}: {r['status']}")

    print(f"\n[云端调度] 第2步：{len(target_lakes)}个湖泊全速并行跑CCM计算...")
    results = list(process_lake_modal.map(target_lakes))
    print("\n=== 全部完成 ===")
    statuses = {}
    for r in results:
        print(f"  {r['lake_name']}: {r['status']}")
        statuses[r["lake_name"]] = r["status"]
    return {"lake_statuses": statuses}


@app.local_entrypoint()
def run_detached(lakes: str = ""):
    """本地只负责把完整流程(ERA5预下载+CCM计算)一次性派发到Modal云端，不在本地等待结果——
    派发完这个函数几秒内就会退出，之后合盖/关终端/断网都不影响云端继续跑。

    用法：
        modal run --detach ccm_modal_app.py::run_detached

    (仍然需要--detach：ephemeral App在本地进程退出时会被Modal服务端自动停掉，不管这次退出
    是正常结束还是断线——--detach就是告诉服务端"本地进程退出后不要停这个App"，没有它，
    run_detached()派发完毕、本地进程自然退出的那一刻，云端刚起步的任务也会被跟着杀掉。)

    第二天想看结果，两种办法都行：
      1. 去Modal网页控制台（这次运行打印出来的 https://modal.com/apps/... 链接）直接看日志/状态；
      2. 随便找一天重新跑一次 `modal run ccm_modal_app.py::process_only`（不用--detach），
         15个湖泊如果都已经跑完，会靠checkpoint秒级返回、打印出全部状态，等于免费问一遍"跑完了吗"。
    """
    target_lakes = lakes.split(",") if lakes else list(LAKES.keys())
    call = run_full_pipeline_modal.spawn(",".join(target_lakes))
    print(f"已派发完整流程(ERA5预下载 + CCM计算)到Modal云端，{len(target_lakes)}个湖泊。")
    print(f"call_id = {call.object_id}")
    print("现在可以直接合盖/关终端，不影响云端继续跑。")
    print("查看进度：去Modal网页控制台看这次运行的App页面；或者明天重跑一次")
    print("  modal run ccm_modal_app.py::process_only")
    print("（不加--detach）——已经跑完的湖泊会靠checkpoint秒级返回状态，等于问一遍进度。")


# ============================================================
# 同步策略说明：
# 上面import的ccm_lib里那一批函数（fetch_lake_water_level、resolve_hylak_id、
# get_basin_mask_cells、build_full_ccm_table、classify_edges等），就是notebook
# 里已经写好的那些函数，原样搬过来存成ccm_lib.py放在同一个目录，Modal打包镜像时
