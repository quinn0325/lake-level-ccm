...
用法
----
    modal run ccm_modal_app.py                              # 全部十个湖
    modal run ccm_modal_app.py --lakes Kalamalka_Lake,Okanagan_Lake
...
两步也可以用 fetch_only / process_only 分开手动跑，见各自 docstring。
...
def main(lakes: str = ""):
    """两步依次跑完：低并发预下载 ERA5，然后全速并行构建面板。

    已缓存过 ERA5 的湖泊第 1 步会很快跳过。要求本地进程全程存活（配合 caffeinate）；
    过夜跑请改用 fetch_only / process_only 分两次手动触发。
    """
    target_lakes = lakes.split(",") if lakes else list(LAKES.keys())

    print(f"第1步：低并发预下载/缓存 ERA5，{len(target_lakes)} 个湖泊: {target_lakes}")
    ...
    if era5_failed:
        ...
        print("       这些湖泊第 2 步大概率会失败在 FAILED_era5_download；若是 CDS 限流被拒，"
              "隔几分钟单独重跑一下 --lakes 就行，不用重跑全部。")

    print(f"\n第2步：{len(target_lakes)} 个湖泊全速并行构建面板...")
